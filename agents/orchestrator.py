...
async def merge_responses(state: OrchestratorState, writer: StreamWriter) -> dict:
    ...
    system_prompt = (
        ...
        "\n"
        "CANLI BİLDİRİMLERİ KORU: Verilerde 'Güncel KAP Bildirimleri:' "
        "başlıklı bir liste varsa AYNEN, madde madde kalır — başlık, tarih ve "
        "bağlantı dahil. Bunu kendi cümlene çevirme, özetleme veya başka bir "
        "bilgiyle birleştirme: bu blok KAP'tan o an çekilmiş ham veridir, "
        "yeniden ifade edilmesi (tarihi/başlığı doğru aktarsan bile) uydurma "
        "riski taşır.\n"
        "\n"
