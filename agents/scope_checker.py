...
def check_scope(query: str) -> dict:
    ...
    if enjeksiyon.get("aktif"):
        ...
        varsayilan_mesaj = mesajlar.get("out_of_scope", {}).get("varsayilan", "Finansal danışmanınız olarak yalnızca desteklenen varlıklar hakkındaki sorularınızı yanıtlayabilirim.")
        if isinstance(varsayilan_mesaj, dict):
            varsayilan_mesaj = varsayilan_mesaj.get("varsayilan", "Finansal danışmanınız olarak yalnızca desteklenen varlıklar hakkındaki sorularınızı yanıtlayabilirim.")
    ...
    if any(k in query_lower for k in destek.get("etiketler", [])):
        mesajlar = scope_config.get("mesajlar", {})
        destek_mesaji = mesajlar.get("out_of_scope", {}).get("destek", "Destek talepleri için Müşteri Hizmetleri ile iletişime geçebilirsiniz.")
        if isinstance(destek_mesaji, dict):
             destek_mesaji = destek_mesaji.get("destek", "Destek talepleri için Müşteri Hizmetleri ile iletişime geçebilirsiniz.")
        return {
            "intent": "OUT_OF_SCOPE",
            "message": destek_mesaji,
            "flags": flags
        }
