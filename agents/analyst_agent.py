...
class AnalystAgent(BaseAgent):
    ...
    async def _fetch_company_data(self, symbol: str, request_message: str) -> str:
        ...
        coros = [
            ...
            self.call_mcp_tool("search_market_news", {"query": f"{symbol} {request_message}", "top_k": 3})
        ]
        
        target_res, price_res, fund_res, news_res = await asyncio.gather(*coros, return_exceptions=True)
        
        lines = [f"--- {symbol} VERİLERİ ---"]
        
        if not isinstance(target_res, Exception) and target_res.get("success") and target_res.get("data"):
            lines.append(f"Hedef Fiyatlar: {json.dumps(target_res['data'], ensure_ascii=False)}")
            
        if not isinstance(price_res, Exception) and price_res.get("success") and price_res.get("data"):
        ...
        if not isinstance(fund_res, Exception) and fund_res.get("success") and fund_res.get("data"):
            lines.append(f"Temel Analiz (F/K, PD/DD vb): {json.dumps(fund_res['data'], ensure_ascii=False)}")
            
        if not isinstance(news_res, Exception) and news_res.get("success") and news_res.get("data"):
            lines.append(f"İlgili Haberler/Belgeler: {json.dumps(news_res['data'], ensure_ascii=False)}")
    ...
    async def execute(self, request: AgentRequest, on_token: Any = None) -> AgentResponse:
        ...
        if symbols:
            ...
            coros.append(self.call_mcp_tool("get_asset_price_history", {"symbols": ["XU100.IS"], "window": "3m"}))
            ...
            if not isinstance(benchmark_res, Exception) and benchmark_res.get("success") and benchmark_res.get("data"):
                context_data.append(f"Endeks Kıyaslaması (XU100 - BIST100 3 Aylık Seyir): {json.dumps(benchmark_res['data'], ensure_ascii=False)}")
