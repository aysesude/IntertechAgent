...
def generate_multi_asset_user():
    ...
    session.add(
        Portfolio(
            ...
            asset_type="Currency",
            symbol="USDTRY",
            quantity=round(random.uniform(1000, 5000), 2),
            avg_cost_try=usd_cost,
        )
    )
    ...
    session.add(
        Portfolio(
            ...
            symbol="GRAM_ALTIN",
            quantity=round(random.uniform(15, 90), 2),
            avg_cost_try=gold_cost,
        )
    )
...
def run_full_multi_asset_demo(user_id: str):
    ...
    print(f"Kullanıcı: {user.full_name} ({user.user_id})")
    print(pd.DataFrame(results))


if __name__ == "__main__":
    demo_user = generate_multi_asset_user()
    run_full_multi_asset_demo(demo_user)
