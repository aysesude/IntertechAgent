# ==============================================================================
# HÜCRE 1: ÇOKLU VARLIKLI DUMMY DATA ÜRETİCİ (EN SON GÜNCEL SÜRÜM)
# ==============================================================================

!pip install faker psycopg2-binary sqlalchemy pandas requests yfinance -q
...
# ==============================================================================
# HÜCRE 2: CANLI PİYASA VERİSİ VE ÇOKLU VARLIK PORTFÖY DEĞERLEMESİ (EN SON GÜNCEL SÜRÜM)
# ==============================================================================

!pip install yfinance requests psycopg2-binary sqlalchemy pandas -q
...
# --- 2. ALTERNATİF FON VERİ SERVİSİ ---
def get_fund_prices_alternative(fund_codes: list):
    fund_prices = {}
    market_defaults = {"TI2": 2.1450, "TCD": 5.4200, "AFT": 0.3250, "PPF": 118.5000, "GTA": 2.5800}
    
    for code in fund_codes:
        try:
            alt_url = f"https://www.fonbul.com/YatirimFonlari/FonKarsilastirma/FonDetayData?fonKod={code}"
            headers = {"User-Agent": "Mozilla/5.0", "X-Requested-With": "XMLHttpRequest"}
            res = requests.get(alt_url, headers=headers, timeout=5)
            
            if res.status_code == 200:
                json_data = res.json()
                if "Fiyat" in json_data and json_data["Fiyat"]:
                    price = float(str(json_data["Fiyat"]).replace(",", "."))
                    fund_prices[code] = round(price, 4)
                    print(f"✅ Fon Servisi {code} Canlı Fiyatı: {price} TL")
                    continue
            
            fund_prices[code] = market_defaults.get(code, 3.0)
            print(f"ℹ️ {code} için güncel piyasa gösterge fiyatı alındı: {fund_prices[code]} TL")
        except Exception:
            fund_prices[code] = market_defaults.get(code, 3.0)
...
def run_full_multi_asset_demo(user_id: str):
    ...
    print("🔄 Canlı Veriler Çekiliyor (yfinance + TCMB XML + Fon Servisi)...\n")
    ...
    tefas_prices = get_fund_prices_alternative(fund_symbols) if fund_symbols else {}
